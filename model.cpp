...
public:
  ...
  pair<vector<double>, vector<vector<double>>> trans_prob(vector<double> state, int action) {
    ...
    if (action == 0) {
      next_state = {{state[0], state[1], state[2] + 1, state[3]},
                    ...
                    {state[0]+cos(random_theta), state[1]+sin(random_theta), state[2], state[3] + random_theta}};
    } else if (action == 1) {
      next_state = {{state[0], state[1], state[2] - 1, state[3]},
                    {state[0], state[1], state[2] + 1, state[3]},
                    {state[0]+cos(random_theta), state[1]+sin(random_theta), state[2], state[3] + random_theta}};
    } else if (action >= 2 && action <= 73) {
      next_state = {{state[0]+cos(theta_vals[action-2]), state[1]+sin(theta_vals[action-2]), state[2], theta_vals[action - 2]},
                    {state[0]+cos(-random_theta), state[1]+sin(-random_theta), state[2], state[3] - random_theta},
                    {state[0]+cos(random_theta), state[1]+sin(random_theta), state[2], state[3] + random_theta}};
    } else {
...
//observation probability function
  ...
  float observation_prob(vector<double> observation, vector<double> state, int action){
  ...
  float reward_function(vector<double> state, int action, vector<double> waypoints){
    float reward=0.0;
...
void update_belief(int action, vector<double> observation) {
    vector<double> weights(num_particles, 1.0); // Initialize weights for each particle to 1.0
    
    // If the "communicate" action is taken, update the belief based on the observation
    if(action == 74) {  // Assuming 74 is your communicate action
        ...
        // If any action other than "communicate" is taken, propagate particles based on the action
        for(int i = 0; i < num_particles; i++) {
            vector<double> particle = particles[i];
            pair<vector<double>, vector<vector<double>>> result=trans_prob(particle, action);
...
int main() {
  ...
  for(int i=0;i<100;i++){
    ...
    pair<vector<double>, vector<vector<double>>> result = testing_obj.trans_prob(s, action);
    vector<double> next_state=result.first;
    cout<<"next state ";
    printVector(next_state);
    vector<double> observation=testing_obj.observation_function(next_state, action);
    cout<<"observation ";
