...
def make_square(x, y, width):
    square = np.array([[x-int(width/2),i] for i in range(y-int(width/2),y+int(width/2))] +\
                      ...
                      [[i,y+int(width/2)] for i in range(x-int(width/2),x+int(width/2))]) 
    return square
original_np_random_normal = np.random.normal
def uniform_like_normal(loc, scale, size=None):
    lower_bound = -45 * np.pi / 180
    upper_bound = 45 * np.pi / 180
    return random.uniform(lower_bound, upper_bound)
np.random.normal = uniform_like_normal
class DataLogger:
    def __init__(self):
        self.path = []
        self.car_state = []
