...
class State:
    def __init__(
        self, budget, is_true_state, waypoint_reached, belief, del_var_particles
    ):
        self.budget = budget
    ...
    def next_state(self, action):
        if action == "communicate" and self.budget > 0:
            # print(self.budget)
            return State(
                self.budget - self.comm_cost,
                True,
                self.waypoint_r(),
                self.belief,
                self.del_var_particles,
            )

        return State(
            self.budget, False, self.waypoint_r(), self.belief, self.del_var_particles
        )
    ...
    def waypoint_r(self):
        if (self.belief[0] - waypoint[0]) ** 2 + (
            self.belief[1] - waypoint[1]
        ) ** 2 < 1:
            return True
