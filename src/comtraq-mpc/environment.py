...
class Environment:
    def __init__(self, obstacles):
        ...
        self.car_struct = np.array(
            [
                [+self.car_length / 20, +self.car_width / 2],
                [+self.car_length / 20, -self.car_width / 2],
                [-self.car_length / 20, -self.car_width / 2],
                [-self.car_length / 20, +self.car_width / 2],
            ],
    ...
    def place_obstacles(self, obs):
        obstacles = (
            np.concatenate(
                [
                    np.array([[0, i] for i in range(100 + 2 * self.margin)]),
                    np.array(
                        [
                            [100 + 2 * self.margin - 1, i]
                    ...
                    np.array(
                        [
                            [i, 100 + 2 * self.margin - 1]
                            for i in range(100 + 2 * self.margin)
                        ]
                    ),
                    obs + np.array([self.margin, self.margin]),
                ]
            )
            * 10
        )
    ...
    def draw_path(self, path):
        ...
        for p in path:
            self.background[
                ...
                p[0] + 10 * self.margin : p[0] + 10 * self.margin + 3,
            ] = color
    ...
    def render(self, x, y, psi, delta):
        ...
        rotated_struct += np.array([x, y]) + np.array(
            [10 * self.margin, 10 * self.margin]
        )
        ...
        gel = np.vstack(
            [
                np.random.randint(-50, -30, 16),
                np.hstack(
                    [np.random.randint(-20, -10, 8), np.random.randint(10, 20, 8)]
                ),
            ]
        ).T
