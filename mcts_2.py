...
def mcts_rollout(node):
    ...
    while True:
        if current_node.is_fully_expanded():
            ...
            next_state, _, reward, done = env.step(action, current_node.state, waypoints)
            if done:
                return reward
...
for i_episode in range(1, max_episodes + 1):
    ...
    while not done:
        steps += 1
        root = MCTSNode(state)
        action = mcts_search(root, iterations=50)
        next_state, _, reward, done = env.step(action, state, waypoints)
        total_reward += reward
        state = next_state
